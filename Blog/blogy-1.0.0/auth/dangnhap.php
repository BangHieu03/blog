            ...
            <div class="form-container">
                ...
                <form method="post">
                    ...
                    if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST['username']) && !empty($_POST['password'])) {
                        ...
                        $user = new user();
                        // Kiểm tra xem tên đăng nhập hoặc email có tồn tại không
                        if ($user->checkUser($username) || $user->checkEmail($username)) {
                            // Nếu tên đăng nhập là email, lấy tên đăng nhập tương ứng
                            if (filter_var($username, FILTER_VALIDATE_EMAIL)) {
                                $username = $user->getUserByEmail($username);
                            }
                            // Kiểm tra mật khẩu
        ...
        input.addEventListener('input', function() {
            ...
            if (input.value.trim() === '') {
                ...
                if (input.getAttribute('type') === 'text') {
                    errorDiv.textContent = 'Tên đăng nhập không được để trống.';
